...
**Input**:
```python
from abyz-lab_workflow_templates import load_template
...
**Scenario**: /abyz-lab:9-feedback 명령으로 버그 리포트 이슈 생성

**Input**:
```bash
# 피드백 명령 실행
/abyz-lab:9-feedback "로그인 시 JWT 토큰이 만료되어도 갱신되지 않음"
```
...
---
Generated by: ABYZ-Lab-ADK Feedback System
...
**Scenario**: ABYZ-Lab-ADK 업데이트 후 사용자 커스터마이징을 보존하며 템플릿 병합
...
```json
{
  ...
  "user_customizations_preserved": [
    {
      "file": ".abyz-lab/config/config.yaml",
      "preserved_fields": [
        "user.name",
  ...
  "conflicts_resolved": [
    {
      "file": ".abyz-lab/config/sections/quality.yaml",
      "resolution": "Merged both changes",
      "details": "User's custom thresholds + new quality metrics"
    }
  ],
  "manual_review_required": [],
  "restoration_point": ".abyz-lab/backups/pre-merge-2025-12-06/"
}
```
...
```json
// .abyz-lab/config/template-history.json
{
...
```bash
# 잘못된 예시 - 템플릿 직접 수정
vim .claude/skills/abyz-lab-workflow-templates/modules/code-templates.md
# → ABYZ-Lab-ADK 업데이트 시 충돌
# → 팀원 간 불일치
```
...
# → .abyz-lab/config/template-overrides.json에 저장
# → 템플릿 업데이트와 독립적
```
...
```
프로젝트 초기화:
/abyz-lab:0-project → 코드 템플릿 선택 → 스캐폴딩 → 버전 추적

피드백 제출:
/abyz-lab:9-feedback → 자동 분류 → 템플릿 적용 → GitHub 이슈 생성
