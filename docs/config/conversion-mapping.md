...
controller:
  ...
  network:
    ...
    dst_port: int            # Host listening port. Default: 7000
    src_port: int            # SoC source port. Default: 7001
...
| `controller.network.dst_port` | `HOST_UDP_PORT` | `uint16_t` | `7000U` |
| `controller.network.src_port` | `SOC_UDP_PORT` | `uint16_t` | `7001U` |
...
#define HOST_UDP_PORT         7000U
#define SOC_UDP_PORT          7001U
...
| `controller.network.dst_port` | `Network.HostUdpPort` | `int` | `7000` |
| `controller.network.src_port` | `Network.SocUdpPort` | `int` | `7001` |
...
Validation Checks:
  ...
  4. BRAM usage ≤ bram_budget_blocks:
       Line buffer: cols × 2 bytes = 6144 bytes per line
       With 4 lines buffered: 24576 bytes = 6.8 BRAM blocks  ✓ (< 50)
```
...
controller:
  network:
    mtu_bytes: 1500       # Standard MTU acceptable at this bandwidth

host:
  network:
    link_speed_gbps: 1    # 1 GbE sufficient
...
# UDP: 1.007 Gbps → 1 GbE at ~100% → use 1500B MTU with tight timing
```
...
# [OK] BRAM estimate: 6.8 blocks (< 50 limit)
