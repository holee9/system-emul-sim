...
| Status | SPEC-GUI-002 교차검증 + 7개 기능 보완 완료 (모든 단위 테스트 217/217 통과) |
|--------|---------------------------------------------------------------|
| Version | 0.5.3 |
...
| 탭 | 구분 | 기능 | 주요 컨트롤 |
|----|------|------|-------------|
| **Tab 1: Panel** | Emulator (HW) | 센서 패널 + X-ray 소스 + 물리 모델 + PDF 추출 | Rows/Cols/BitDepth/PixelPitch, kVp/mAs/ExposureTimeMs, TestPattern(Counter/Checkerboard/PhysicsBased/FlatField)/Seed, BtnLoadPdf/Apply |
| **Tab 2: FPGA** | Emulator (HW) | CSI-2 인터페이스 + Gate Timing + FSM + SPI 레지스터 맵 | Lanes/DataRate/LineBuffer, GateLines/LineTime, FSM 상태 표시, SPI 14개 레지스터 표 |
| **Tab 3: SoC** | Emulator (HW) | Frame Buffer + UDP TX + Sequence Engine 상태 | BufferCount, TargetIP/Port, SequenceEngineState (Idle/Arming/Acquiring), FramesProcessed/Failed |
| **Tab 4: Ethernet** | Emulator (HW) | 네트워크 채널 에뮬레이션 + 지연 제어 | PacketLossRate/ReorderRate/CorruptionRate, MinDelayMs/MaxDelayMs (파이프라인 실시간 반영), 실시간 통계 |
| **Tab 5: Host PC** | Simulator (SW) | SDK 연결 + 프레임 저장 설정 + 최근 저장 목록 | ClientMode, OutputDirectory/Format, AutoReconnect, DropRate%, RecentSaves 최대 10개 |
...
- **217/217** unit tests passing (GUI.Application) — 이전 실패 3개(HelpViewModel) + ScenarioRunner 모두 수정 완료
...
| **SPEC-GUI-002** | ✅ Complete | 100% | 모듈 지향 탭 아키텍처 — 6탭 HW/SW 구조 전면 개편 + 교차검증 7개 기능 보완 (SPI 레지스터 맵, Sequence Engine, Recent Saves, TestPattern/Seed, MinDelayMs 파이프라인 연결 등) |
...
> **SPEC-GUI-002 교차검증 + 7개 기능 보완 완료 ✅** (2026-03-18)
>
> - **HelpViewModel async 버그 수정**: `async void OnSelectTopic` → `void` + 동기 `Convert()` 전환 → HelpViewModel 테스트 3개 통과
> - **ScenarioRunner 타임아웃 수정**: `SyncProgress<T>` 내부 클래스 도입 + 30초 타임아웃 → 1024×1024 파이프라인 초기화 시간 커버
> - **네트워크 MinDelay/MaxDelay 파이프라인 연결**: `NetworkChannel.SetDelayRange()` 추가 → `SimulatorPipeline.SetDelayRange()` → `PipelineDetectorClient.UpdateNetworkConfig()` → MainViewModel 속성 변경 시 실시간 반영
> - **Panel IsReady 검증 강화**: `IsReady => true` → 실제 `PanelRows > 0 && PanelCols > 0 && BitDepth in [8,16]` 조건 검증 + PropertyChanged 구독
> - **TestPattern / ExposureTimeMs / Seed UI 연동**: SimulatorControlViewModel에 3개 프로퍼티 추가, ToDetectorConfig/UpdateFromConfig 반영, PanelEmulatorView에 Physics Model GroupBox 추가
> - **FPGA SPI 레지스터 맵 표시**: `SpiRegisterEntry` 레코드 + 14개 레지스터 (§6.3 기반) ListView 테이블 — STATUS, CONTROL, FRAME_COUNT, TIMING_*, PANEL_*, CSI2_LANE, ERROR_FLAGS
> - **SoC Sequence Engine 상태 표시**: `UpdateSequenceState(isConnected, isAcquiring)` → Idle/Arming/Acquiring 3상태 UI 연동
> - **SDK Recent Saves 목록**: `ObservableCollection<string>` 최대 10개, SaveFrameAsync 저장 시 자동 추가, IntToVisibilityConverter로 빈 상태 안내 표시
> - **전체 단위 테스트 217/217 통과** (기존 3개 실패 → 0개 실패)
>
...
┌─────────────────────────────────────────────────────────────────────┐
│                        GUI.Application (WPF)                       │
├──────────────────── Pipeline Toolbar (항상 노출) ───────────────────┤
│  [●Panel ✅] [●FPGA ✅] [●SoC ✅] [●ETH ✅] [●SDK ✅]  [▶ Integrate & Run]  [■ Stop All]  │
├─────────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌───────────┐ │
│  │   Tab 1     │  │   Tab 2     │  │   Tab 3     │  │   Tab 4   │ │
│  │   Panel     │  │    FPGA     │  │     SoC     │  │ Ethernet  │ │
...
#### Tab 1: Panel (Emulator — HW)

**Sensor Spec:**
- Rows, Cols, BitDepth, PixelPitchUm
...
**Physics Model:**
- TestPattern: Counter / Checkerboard / PhysicsBased / FlatField (ComboBox)
- PhysicsBased: ScintillatorModel → GateResponseModel → ExposureModel 3단계 체인 (kVp/mAs 실제 반영)
- Seed: 재현 가능한 랜덤 시뮬레이션용 시드 값

**PDF 추출 서브패널:**
- BtnLoadPdf / Apply — sensor panel spec 자동 추출

**Ready 조건:** Rows > 0 && Cols > 0 && BitDepth in [8, 16]

#### Tab 2: FPGA (Emulator — HW)

**CSI-2 인터페이스:**
- Lanes (1/2/4), DataRateMbps, LineBufferDepth

**Gate Timing:**
- GateLines, LineTime, FrameTime, ROICSettleUs, AdcConvUs

**SPI 레지스터 맵 (14개 레지스터 표 — §6.3 기반):**

| 레지스터 | 주소 | 설명 |
|---------|------|------|
| STATUS | 0x20 | FSM 상태 플래그 |
| CONTROL | 0x21 | start_scan, reset 비트 |
| FRAME_COUNT_HI/LO | 0x30/0x31 | 프레임 카운터 (16-bit) |
| TIMING_ROW_PERIOD | 0x40 | 행 주기 |
| TIMING_GATE_ON/OFF | 0x41/0x42 | Gate On/Off 시간 |
| ROIC_SETTLE_US | 0x43 | ROIC 정착 시간 |
| ADC_CONV_US | 0x44 | ADC 변환 시간 |
| FRAME_BLANK_US | 0x45 | 프레임 블랭킹 시간 |
| PANEL_ROWS/COLS | 0x50/0x51 | 패널 해상도 |
| CSI2_LANE_COUNT | 0x60 | CSI-2 레인 수 |
| ERROR_FLAGS | 0x80 | CSI2_SYNC, FRAME_DROP, CRC_FAIL 등 |

**FSM 상태 표시:** IDLE / INTEGRATE / READOUT / LINE_DONE / FRAME_DONE / ERROR

#### Tab 3: SoC (Emulator — HW)

**CSI-2 RX:**
- 수신 레인, CRC-16 검증 on/off

**Frame Buffer:**
- BufferCount (1-8), 버퍼 상태: Free/Filling/Ready/Sending

**UDP TX:**
- TargetIP, TargetPort (1024-65535)

**Sequence Engine 상태:**
- `Idle` (미연결) → `Arming` (연결됨, 취득 전) → `Acquiring` (취득 중)
- MainViewModel.IsConnected/IsAcquiring 변경 시 자동 반영

#### Tab 4: Ethernet (Emulator — HW)

**네트워크 채널 파라미터:**
- Packet Loss Rate: 0.0-1.0
- Reorder Rate: 0.0-1.0
- Corruption Rate: 0.0-1.0
- **MinDelayMs / MaxDelayMs**: 변경 즉시 `SimulatorPipeline.SetDelayRange()` → `NetworkChannel`에 실시간 반영

**실시간 통계:**
- PacketsSent, PacketsLost, PacketsReordered, PacketsCorrupted
- 처리량 (Gbps), 연결 상태

#### Tab 5: Host PC (Simulator — SW)

**SDK 연결:**
- ClientMode: SimulatedDetectorClient / PipelineDetectorClient
- SDK 연결 상태, FramesReceived, DroppedFrames, DropRate%

**저장 설정:**
- OutputDirectory, Format (TIFF/RAW), MaxFrames
- AutoReconnect on/off, MaxRetries

**Recent Saves (최근 저장 목록):**
- `SaveFrameAsync` 저장 성공 시 자동 추가
- 최대 10개 유지 (중복 제거 후 최신 우선)
- 목록이 비어있으면 "(no recent saves)" 안내 표시

#### Tab 6: Console (Simulator — SW / 영상 Viewer)

**영상 뷰어:**
- 16-bit 그레이스케일 실시간 렌더링 (WriteableBitmap)
- Window/Level 슬라이더, Min/Max/Mean 통계

**획득 제어:**
- Start / Stop Acquisition, 전체 파이프라인 Start/Stop/Reset

**시나리오 러너:**
- IT01-IT19 통합 테스트 시나리오 실행
- 진행률 표시 (프로그레스 바), PASS/FAIL 결과 및 상세 메시지
...
├── PanelEmulatorView.xaml  ├── PanelEmulatorViewModel       ├── PipelineDetectorClient
├── FpgaEmulatorView.xaml   ├── FpgaEmulatorViewModel        ├── SimulatorPipeline
├── SocEmulatorView.xaml    ├── SocEmulatorViewModel         ├── ScenarioRunner
├── EthernetView.xaml       ├── EthernetViewModel            └── NetworkChannel
├── SdkHostView.xaml        ├── SdkHostViewModel
├── ConsoleView.xaml        ├── ConsoleViewModel
└── HelpView.xaml           ├── StatusViewModel
...
└── tools/GUI.Application/         # 통합 WPF GUI (4계층 에뮬레이터) ✅ 완료 (217/217 tests)
```
...
| **GUI.Application** | ✅ 완료 | 85%+ | 217 passing (0 실패) |
| **meta-detector** (Yocto) | ✅ 완료 | - | 레시피 완료 |
| **IntegrationTests** | ✅ 완료 | - | 250 passing (4 skipped) |
| **합계** | **M2~M5+GUI 완료** | **85%+** | **1,553+ passing (4 skipped)** |
